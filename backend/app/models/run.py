...
from app.utils.json_safe import SafeJSON
...
class Run(Base):
    ...
    input: Mapped[dict] = Column(SafeJSON, default=dict)
    output: Mapped[dict] = Column(SafeJSON, default=dict)
    error: Mapped[str] = Column(Text, nullable=True)
    duration_ms: Mapped[int] = Column(Integer, nullable=True)
    node_results: Mapped[list] = Column(SafeJSON, nullable=True)
